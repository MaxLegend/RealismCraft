...
public class RGenLayerDeepOcean extends RGenLayer
{
    public RGenLayerDeepOcean(long seed, RGenLayer parent)
    {
        super(seed);
        this.parent = parent;
    }
...
//        validateIntArray(parentIDs, xSize, zSize);
        ...
        for (int z = 0; z < parZSize; ++z)
        {
            for (int x = 0; x < parXSize; ++x)
            {
                ...
                {
                    outCache[outIndex] = Biome.getIdForBiome(RegBiomes.DEEP_OCEAN);
                }
                else
                {
