...
public class AshFL extends BlockFruitLeaves {

	public AshFL(Material materialIn, String name, float hardness, float resistanse, SoundType soundtype, CreativeTabs tab) {
	...
	public void fruitUpdate(World w, BlockPos p, IBlockState s, Random r, IDate date) {
		...
		if(date.getMonth() < 3 || date.getMonth() == 11) {
			w.setBlockState(p, BlocksList.REAL_ASH_LEAVES.getDefaultState());
		}
		//дефолтное состо€ние - 3 - апрель, цветение
		//с апрел€ по август - незрелые плоды
		if(date.getMonth() > 3 && date.getMonth() < 8) {
			w.setBlockState(p, s.withProperty(STAGE, 1));
		} 
		//c сент€бр€ по но€брь созревшие крылатки €сен€
