...
public class GenLarchTree  extends WorldGenerator {

	private final  IBlockState LOG = BlocksList.REAL_LARCH.getDefaultState().withProperty(BlockRealTrees.STAGE, 6);
	private  IBlockState THICK_BRANCH ;
	
	private final  IBlockState LEAVES = BlocksList.REAL_LARCH_LEAVES.getDefaultState();
	...
	public boolean generateTree(World w, BlockPos pos) {
		...
		THICK_BRANCH = BlocksList.REAL_TB_LARCH.getDefaultState();
		if (pos.getY() >= 1 && pos.getY() + heightTree + 1 <= 256) {
			...
			else
			{
				...
				if(state.getBlock() instanceof GrassSided || state.getBlock() instanceof BlockDirt) {
					...
					for (int heightFoliageGrowth = pos.getY() -7 + heightTree; heightFoliageGrowth <= pos.getY() + heightTree; ++heightFoliageGrowth)
					{
						int k2 = heightFoliageGrowth - (pos.getY() + heightTree -2);// —тартовый размер каждого сло€ кроны(радиус каждого сло€ конуса)
