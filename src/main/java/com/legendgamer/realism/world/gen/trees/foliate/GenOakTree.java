...
public class GenOakTree  extends WorldGenerator {

	private final  IBlockState LOG = BlocksList.REAL_OAK.getDefaultState().withProperty(BlockRealTrees.STAGE, 6);
	private  IBlockState THICK_BRANCH ;
	private final  IBlockState BRANCH = BlocksList.REAL_OAK_BRANCH.getDefaultState();
	private final  IBlockState LEAVES = BlocksList.REAL_OAK_LEAVES.getDefaultState();

	public boolean generateTree(World w, BlockPos pos) {

		
		ThreadLocalRandom tr = ThreadLocalRandom.current();
		int heightTree = ThreadLocalRandom.current().nextInt(6, 10);
		boolean checkOppGrowth = true;
		THICK_BRANCH = BlocksList.REAL_TB_OAK.getDefaultState();
		if (pos.getY() >= 1 && pos.getY() + heightTree + 1 <= 256) {
			...
			else
			{
				...
				if(state.getBlock() instanceof GrassSided || state.getBlock() instanceof BlockDirt) {
					...
					for (int heightFoliageGrowth = pos.getY() -4 + heightTree; heightFoliageGrowth <= pos.getY() + heightTree; ++heightFoliageGrowth)
					{
						int k2 = heightFoliageGrowth - (pos.getY()+ 1 + heightTree);// —тартовый размер каждого сло€ кроны(радиус каждого сло€ конуса)
