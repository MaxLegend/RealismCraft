...
public class GenLindenTree  extends WorldGenerator {

	private final  IBlockState LOG = BlocksList.REAL_LINDEN.getDefaultState().withProperty(BlockRealTrees.STAGE, 6);
	private  IBlockState THICK_BRANCH ;
	private final  IBlockState BRANCH = BlocksList.REAL_LINDEN_BRANCH.getDefaultState();
	private final  IBlockState LEAVES = BlocksList.REAL_LINDEN_LEAVES.getDefaultState();
	...
	public boolean generateTree(World w, BlockPos pos) {
		...
		THICK_BRANCH = BlocksList.REAL_TB_LINDEN.getDefaultState();
		if (pos.getY() >= 1 && pos.getY() + heightTree + 1 <= 256) {
			...
			else
			{
				...
				if(state.getBlock() instanceof GrassSided || state.getBlock() instanceof BlockDirt) {
					...
					{
						int k2 = heightFoliageGrowth - (pos.getY() + heightTree);// —тартовый размер каждого сло€ кроны(радиус каждого сло€ конуса)
