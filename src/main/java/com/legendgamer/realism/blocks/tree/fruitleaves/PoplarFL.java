...
public class PoplarFL extends BlockFruitLeaves {

	public PoplarFL(Material materialIn, String name, float hardness, float resistanse, SoundType soundtype, CreativeTabs tab) {
	...
	public void fruitUpdate(World w, BlockPos p, IBlockState s, Random r, IDate date) {
		...
		if(date.getMonth() < 5 || date.getMonth() == 11) {
			w.setBlockState(p, BlocksList.REAL_POPLAR_LEAVES.getDefaultState());
		}
		//дефолтное состо€ние - 5 - июнь, цветение
		//с июл€ по сент€брь - незрелые плоды
