...
public class GenPineTree  extends WorldGenerator {

	private final  IBlockState LOG = BlocksList.REAL_PINE.getDefaultState().withProperty(BlockRealTrees.STAGE, 6);
	private  IBlockState THICK_BRANCH ;
	
	private final  IBlockState LEAVES = BlocksList.REAL_PINE_LEAVES.getDefaultState();

	public boolean generateTree(World w, BlockPos pos) {

		
		ThreadLocalRandom tr = ThreadLocalRandom.current();
		int heightTree = ThreadLocalRandom.current().nextInt(20, 29);
		boolean checkOppGrowth = true;
		THICK_BRANCH = BlocksList.REAL_TB_PINE.getDefaultState();
		if (pos.getY() >= 1 && pos.getY() + heightTree + 1 <= 256) {
			...
			else
			{
				...
				if(state.getBlock() instanceof GrassSided || state.getBlock() instanceof BlockDirt) {
					...
					for (int heightFoliageGrowth = pos.getY() -3 + heightTree; heightFoliageGrowth <= pos.getY() + heightTree; heightFoliageGrowth++)
					{
						int k2 = heightFoliageGrowth - (pos.getY() + heightTree+4);// —тартовый размер каждого сло€ кроны(радиус каждого сло€ конуса)
						int l2 = 1 - k2 / 3;// –адиус нижнего сло€ конуса
