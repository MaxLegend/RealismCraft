...
public class GenBirchTree  extends WorldGenerator {

	private final  IBlockState LOG = BlocksList.REAL_BIRCH.getDefaultState().withProperty(BlockRealTrees.STAGE, 6);
	private  IBlockState THICK_BRANCH ;
	private final  IBlockState BRANCH = BlocksList.REAL_BIRCH_BRANCH.getDefaultState();
	private final  IBlockState LEAVES = BlocksList.REAL_BIRCH_LEAVES.getDefaultState();

	public boolean generateTree(World w, BlockPos pos) {

		
		ThreadLocalRandom tr = ThreadLocalRandom.current();
		int heightTree = ThreadLocalRandom.current().nextInt(9, 15);
		boolean checkOppGrowth = true;
		THICK_BRANCH = BlocksList.REAL_TB_BIRCH.getDefaultState();
		if (pos.getY() >= 1 && pos.getY() + heightTree + 1 <= 256) {
			...
			else
			{
				...
				if(state.getBlock() instanceof GrassSided || state.getBlock() instanceof BlockDirt) {
					...
					{
						int k2 = heightFoliageGrowth - (pos.getY() + heightTree + 1);// —тартовый размер каждого сло€ кроны(радиус каждого сло€ конуса)
						int l2 = 1 - k2 / 4;// –адиус нижнего сло€ конуса
