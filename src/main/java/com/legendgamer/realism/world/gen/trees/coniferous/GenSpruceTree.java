...
public class GenSpruceTree  extends WorldGenerator {

	private final  IBlockState LOG = BlocksList.REAL_SPRUCE.getDefaultState().withProperty(BlockRealTrees.STAGE, 6);
	private  IBlockState THICK_BRANCH ;
	
	private final  IBlockState LEAVES = BlocksList.REAL_SPRUCE_LEAVES.getDefaultState();

	public boolean generateTree(World w, BlockPos pos) {

		
		ThreadLocalRandom tr = ThreadLocalRandom.current();
		int heightTree = ThreadLocalRandom.current().nextInt(13, 16);
		boolean checkOppGrowth = true;
		THICK_BRANCH = BlocksList.REAL_TB_SPRUCE.getDefaultState();
		if (pos.getY() >= 1 && pos.getY() + heightTree + 1 <= 256) {
			...
			else
			{
				...
				if(state.getBlock() instanceof GrassSided || state.getBlock() instanceof BlockDirt) {
							...
							if(j2 > 5 && !w.isAirBlock(pos.up(2))) {
								...
								for(EnumFacing f : EnumFacing.HORIZONTALS) {
									...
									if(f == EnumFacing.EAST && tr.nextInt(2) == 1) {
										int randStage = tr.nextInt(0,1);
										for(int stage = randStage; stage >= 0; stage--) {
										
											this.setBlockAndNotifyAdequately(w, pos.up(j2).offset(f,stage-randStage-1), THICK_BRANCH.withProperty(BlockThickBranch.STAGE, 1).withProperty(BlockThickBranch.AXIS, BlockThickBranch.EnumAxis.X));
										}
									} 
									...
									if(f == EnumFacing.SOUTH && tr.nextInt(2) == 1) {
										...
										for(int stage = randStage; stage >= 0; stage--) {
											...
											this.setBlockAndNotifyAdequately(w, pos.up(j2).offset(f,stage-randStage-1), THICK_BRANCH.withProperty(BlockThickBranch.STAGE, 1).withProperty(BlockThickBranch.AXIS, BlockThickBranch.EnumAxis.Z));
										}
									} 
					...
					for (int heightFoliageGrowth = pos.getY() -6 + heightTree; heightFoliageGrowth <= pos.getY() + heightTree; ++heightFoliageGrowth)
					{
						int k2 = heightFoliageGrowth - (pos.getY() + heightTree -4);// —тартовый размер каждого сло€ кроны(радиус каждого сло€ конуса)
					int l2 = 1 - k2 / 2;// –адиус нижнего сло€ конуса
